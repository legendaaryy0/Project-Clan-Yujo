...
// Reverted to root collections for your specific project
...
const PollWidget = ({ poll, currentUser, logActivity, isAdmin, onDelete }) => {
    if (!poll) return null;
    ...
    const hasReachedLimit = userVoteCount >= 2;
                ...
                {optionsWithVoters.map((opt, idx) => {
                    ...
                    return (
                        <button 
                            ...
                            disabled={isDisabled}
                            className={`w-full relative h-12 rounded-xl overflow-hidden border transition-all text-left ${
                                isCurrentlyVoted 
                        ...
                        >
                            ...
                            <div className="absolute inset-0 flex justify-between items-center px-4">
                                <span className={`font-bold text-sm ${isCurrentlyVoted ? 'text-white' : 'text-zinc-300'}`}>{opt.text}</span>
...
const CinemaControlPage = ({ screenings, moviePool, isAdmin, logActivity, currentUser }) => {
    ...
    const createMoviePoll = async () => {
        ...
        const options = pollSelection.map(m => ({ text: m.title, votes: 0, voters: [] }));
        await addDoc(getCollectionRef(COLLECTION_POLLS), { question: "Which movie for Friday Night? (Select up to 2)", options: options, votedBy: [], status: 'active', createdAt: serverTimestamp() });
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => { 
        if (user) { 
            const docRef = getDocRef(COLLECTION_USERS, user.uid);
  ...
  const handleLogout = () => { auth.signOut(); setCurrentUser(null); setActiveTab('dashboard'); };
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-200 font-sans flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="hidden lg:flex w-72 bg-zinc-950 border-r border-zinc-900 flex-col relative z-20 justify-between p-0">
          ...
          <nav className="flex-1 py-8 space-y-2 px-4">
              {[{ id: 'dashboard', icon: LayoutDashboard, label: 'Headquarters' },
                ...
                { id: 'cinema', icon: Ticket, label: 'Cinema Control', admin: true },
      ...
      <div className="lg:hidden fixed bottom-6 left-1/2 transform -translate-x-1/2 w-[90%] max-w-sm bg-zinc-950/90 backdrop-blur-2xl border border-zinc-800 rounded-full z-40 flex justify-around items-center p-1.5 shadow-2xl shadow-black/50">
          {[{ id: 'dashboard', label: 'Home', icon: LayoutDashboard },{ id: 'cinema', label: 'Cinema', icon: Ticket, admin: true }].map(item => (
              (!item.admin || currentUser.role === 'admin') && (
          ...
          {[{ id: 'projects', label: 'Work', icon: Briefcase }, { id: 'nexus', label: 'Clan', icon: Users }].map(item => (
              <button key={item.id} onClick={() => setActiveTab(item.id)} className={`p-3.5 rounded-full transition-all duration-300 relative group ${activeTab === item.id ? 'bg-white text-black shadow-lg scale-110' : 'text-zinc-500 hover:text-white'}`}><item.icon size={20} strokeWidth={2.5} /></button>
          ))}
      </div>
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-10 relative bg-black scroll-smooth pb-32 lg:pb-10">
          ...
          {activeTab === 'cinema' && <CinemaControlPage screenings={screenings} moviePool={moviePool} isAdmin={currentUser.role === 'admin'} logActivity={logActivity} currentUser={currentUser} />}
